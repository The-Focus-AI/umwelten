...
/**
 * Complex reasoning test with Qwen3 14b
 *
 * Run with: npx tsx src/test/test-reasoning-complex.ts
 */
...
async function main() {
  ...
  try {
    ...
    if (codeMatch) {
      ...
      console.log('-'.repeat(30));
      console.log(codeMatch[1].slice(0, 500) + (codeMatch[1].length > 500 ? '...' : ''));
  ...
  const report = adaptSingleTestResult('Complex Reasoning Streaming', success, {
    title: 'Complex Reasoning Streaming Test',
  ...
  const filepath = await reporter.toFile(report, 'reasoning-complex-report.md');
