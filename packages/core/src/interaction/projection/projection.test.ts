...
import type { NormalizedSessionEntry, NormalizedSession } from '../types/normalized-types.js';
import type { SourceSessionKind } from '../types/domain-types.js';
import type { SessionAdapter } from '../adapters/adapter.js';

// ── Fixtures ────────────────────────────────────────────────────────────

function makeEntry(overrides: Partial<NormalizedSessionEntry> = {}): NormalizedSessionEntry {
  return {
...
function makeFullSession(overrides: Partial<NormalizedSession> = {}): NormalizedSession {
  return {
    id: 'pi-session-001',
    source: 'pi',
...
describe('toSourceSession', () => {
  it('converts a claude-code entry to a SourceSession', () => {
    ...
    expect(source.firstPrompt).toBe('The auth token is expiring before the refresh completes');
  });
  ...
  it('carries sourceData through', () => {
    const entry = makeEntry({ sourceData: { filename: 'session.jsonl', displayName: 'Refactor' } });
    const source = toSourceSession(entry, 'pi');
...
describe('projectSessionEntry', () => {
  it('creates a default Exploration from a session entry', () => {
    ...
    expect(exploration.name).toBe('The auth token is expiring before the refresh completes');
...
describe('projectSessions', () => {
  ...
  it('handles empty project with no sessions', async () => {
    ...
    const result = await projectSessions('/projects/empty', { registry: emptyRegistry });
  ...
  it('handles adapters that error gracefully', async () => {
    const errorAdapter: SessionAdapter = {
      ...
      discoverSessions: async () => { throw new Error('Adapter unavailable'); },
      getSessionEntry: async () => null,
      getSession: async () => null,
      getMessages: async () => [],
    ...
    const result = await projectSessions('/projects/my-app', { registry: errorRegistry });
