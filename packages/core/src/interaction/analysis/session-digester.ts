...
export function filterBeats(
	rawBeats: ReturnType<typeof messagesToBeats>,
): ReturnType<typeof messagesToBeats> {
	...
	for (const beat of rawBeats) {
		...
		if (
			userContent.includes("<ide_opened_file>") &&
			stripXmlTags(userContent).length > 10
		) {
...
SKILL_CANDIDATES: Patterns or procedures that worked — "to do X, you need to Y." Things someone could follow in the future.
...
function parseBatchResponse(
	...
	for (let i = 1; i <= beatCount; i++) {
		const regex = new RegExp(
			`Beat\\s+${i}[:\\s]+(.+?)(?=Beat\\s+\\d|Through|FACTS|$)`,
			"is",
		);
	...
	function extractCategory(label: string, nextLabels: string[]): string[] {
		const nextPattern = nextLabels
			...
			.join("|");
		const regex = new RegExp(
			`${label.replace(/_/g, "[_ ]")}[:\\s]*([\\s\\S]*?)(?=${nextPattern}|$)`,
			"i",
		);
		const match = text.match(regex);
		if (!match) return [];
	...
	const learnings: ClassifiedLearnings = {
		facts: extractCategory("FACTS", [
			"SKILL_CANDIDATES",
			"MISTAKES",
			"OPEN_LOOPS",
			"PREFERENCES",
		]),
		skill_candidates: extractCategory("SKILL_CANDIDATES", [
			"MISTAKES",
			"OPEN_LOOPS",
			"PREFERENCES",
		]),
	...
	return {
		beatNarratives,
		throughLine: throughLine || text.slice(0, 200),
		learnings,
	};
}
...
async function compactViaStrategy(
	...
	const stimulus = new Stimulus({
		...
		instructions: [
			"Summarize the conversation segment. Output: 1) Through-line (2-4 sentences) 2) Key facts (bullet points)",
		],
		runnerType: "base",
	});
...
export async function digestSession(
	...
	try {
		...
		console.log(
			`  ${rawBeats.length} raw beats → ${beats.length} after filtering`,
		);
		...
		const digestBeats: DigestBeat[] = beats.map((beat, i) => {
			...
			const assistantMsgs = beat.messages.filter(
		...
		onProgress?.({
			phase: "compacting",
			detail: `${digestBeats.length} beats in groups of ${BEATS_PER_BATCH}`,
		});
		...
		for (let gi = 0; gi < digestBeats.length; gi += BEATS_PER_BATCH) {
			...
			const beatsText = group
				.map((b, i) => {
					const toolStr =
						b.toolsUsed.length > 0
			...
			try {
				...
				for (let i = 0; i < group.length; i++) {
					group[i].narrative =
						batchParsed.beatNarratives[i] || batchParsed.throughLine;
				...
				const totalExtracted = Object.values(batchParsed.learnings).reduce(
					(a, b) => a + b.length,
					0,
				);
				console.log(
		...
		console.error(
			`Failed to digest session ${session.sessionId}:`,
			err instanceof Error ? err.message : err,
		);
		return null;
	}
...
async function detectPhases(
	...
	const beatSummaries = beats
		.map(
			(b, i) => `Beat ${i + 1}: ${b.narrative || b.userRequest.slice(0, 100)}`,
		)
		.join("\n");
...
${beatSummaries}`;
	...
	const stimulus = new Stimulus({
		...
		instructions: [
			"Identify distinct phases in this conversation. Be concise.",
		],
		runnerType: "base",
	});
	const interaction = new Interaction(model, stimulus);
	...
	const phaseRegex =
		/Phase\s+\d+:\s*(.+?)\s*\(beats?\s*(\d+)\s*[-–]\s*(\d+)\)\s*\n([^\n]+)/gi;
	let match;
	...
	if (phases.length === 0) {
		...
		for (const line of lines) {
			const nameMatch = line.match(
				/(?:Phase\s+\d+[:\s]*)?(.+?)\s*\(beats?\s*(\d+)\s*[-–]\s*(\d+)\)/i,
			);
			if (nameMatch) {
 ...
 */
async function isSessionDigested(
	projectPath: string,
	sessionId: string,
): Promise<boolean> {
	try {
...
export async function digestProject(
	...
	for (let i = 0; i < toProcess.length; i++) {
		...
		if (verbose) {
			const rawPrompt = (session.firstPrompt || "(no prompt)")
				.replace(/<[^>]+>/g, "")
				.trim();
		...
		if (digest) {
			...
			if (verbose)
				console.log(
					`    ✓ ${digest.segments.length} segments, ${digest.allFacts.length} facts`,
				);
		} else {
			failed++;
		}
...
async function buildSessionContext(options: {
	...
	const sessionContext = beats
		.map((beat, i) => {
			...
			const toolStr =
				uniqueTools.length > 0 ? ` [tools: ${uniqueTools.join(", ")}]` : "";
...
Answer the user's questions based on this context. Be specific — reference beat numbers, quote decisions, and cite extracted facts, mistakes, and skill candidates when relevant. You can have a multi-turn conversation about this session.
...
export async function buildSessionAnalysisInteraction(options: {
	...
	const stimulus = new Stimulus({
		role: "session analyst",
		objective:
			"answer questions about a completed coding session in an interactive conversation",
		instructions: [systemPrompt],
		runnerType: "base",
	});
...
export async function digestAllProjects(
	...
	for (let i = 0; i < projectPaths.length; i++) {
		...
		try {
			const result = await digestProject(projectPath, model, modelUsed, {
				force,
				verbose,
			});
			...
			if (verbose && result.digested > 0) {
				console.log(
					`  → ${result.digested} digested, ${result.skipped} skipped, ${result.failed} failed\n`,
				);
			} else if (verbose) {
