...
describe('SavedExplorationStore', () => {
  ...
  it('persists the manifest as readable JSON', async () => {
    ...
    const manifestPath = join(testDir, '.umwelten', 'explorations', `${slug}.json`);
  ...
  it('opens a saved exploration by slug', async () => {
    const session = makeSession();
    const { slug } = await store.save(createVirtualExploration('slug-test', [session]));
  ...
  it('deletes a saved exploration by slug', async () => {
    const session = makeSession();
    const { slug } = await store.save(createVirtualExploration('delete-me', [session]));
  ...
  it('handles missing version field in manifest', async () => {
    ...
    const dir = join(testDir, '.umwelten', 'explorations');
    await mkdir(dir, { recursive: true });
    await writeFile(join(dir, 'no-version.json'), JSON.stringify({ id: 'test', name: 'test' }), 'utf-8');
  ...
  it('store creates directory lazily on first save', async () => {
    ...
    await expect(readdir(join(testDir, '.umwelten', 'explorations'))).rejects.toThrow();
