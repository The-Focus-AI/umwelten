...
describe('PromotionRouter', () => {
  ...
  it('promotes to FACTS.md', async () => {
    const result = await router.promote({
      target: 'project-fact', confidence: 0.7, label: 'Fact',
      content: 'The project uses pnpm workspaces', title: 'PNPM',
      source: 'heuristic',
    });
  ...
  it('promotes to saved reflection', async () => {
    const result = await router.promote({
      target: 'saved-reflection', confidence: 0.6, label: 'Reflection',
      content: 'Key insight: the memory runner pattern conflicted with context builder.',
      title: 'Memory Runner Insight', source: 'heuristic',
    });
  ...
  it('promotes to artifact', async () => {
    const result = await router.promote({
      target: 'artifact', confidence: 0.5, label: 'Artifact',
      content: '# Weekly Report\n\nProgress on features.', title: 'Weekly Report',
      source: 'heuristic',
    });
  ...
  it('promotes to user model', async () => {
    const result = await router.promote({
      target: 'user-model', confidence: 0.5, label: 'User model',
      content: 'Prefers TypeScript strict mode', title: 'TypeScript',
      source: 'heuristic',
    });
  ...
  it('promotes to ADR', async () => {
    const result = await router.promote({
      target: 'adr', confidence: 0.9, label: 'ADR',
      content: 'We decided to use pnpm over npm for workspace management.',
      title: 'Use pnpm for workspaces', source: 'heuristic',
    });
  ...
  it('promotes to skill draft', async () => {
    const result = await router.promote({
      target: 'skill', confidence: 0.7, label: 'Skill',
      content: '# My Skill\n\nSteps to reproduce.',
      title: 'My Skill', source: 'heuristic',
    });
  ...
  it('promotes to domain language (CONTEXT.md)', async () => {
    const result = await router.promote({
      target: 'domain-language', confidence: 0.6, label: 'Domain language',
      content: 'An Exploration is a queryable grouping of Source Sessions.',
      title: 'Exploration definition', source: 'heuristic',
    });
  ...
  it('reports failure gracefully', async () => {
    const brokenRouter = new PromotionRouter({ projectRoot: '/nonexistent/path/that/cannot/be/created' });
    const result = await brokenRouter.promote({
      target: 'saved-reflection', confidence: 0.5, label: 'Test',
      content: 'Test content', title: 'Test', source: 'heuristic',
    });
  ...
  it('promotes multiple decisions', async () => {
    const decisions = [
      { target: 'project-fact' as const, confidence: 0.8, label: 'Fact',
        content: 'Project uses pnpm', title: 'PNPM', source: 'heuristic' as const },
      { target: 'saved-reflection' as const, confidence: 0.5, label: 'Reflection',
        content: 'Learned about pnpm', title: 'PNPM Learning', source: 'heuristic' as const },
    ];
