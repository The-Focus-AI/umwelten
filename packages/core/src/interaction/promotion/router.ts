...
export class PromotionRouter {
  ...
  private async promoteToAgentInstruction(
    ...
    const filePath = this.config.agentInstructionFile ?? join(this.config.projectRoot, 'AGENTS.md');
  ...
  private async promoteToFact(content: string): Promise<PromotionResult> {
    const filePath = this.config.factsFile ?? join(this.config.projectRoot, 'FACTS.md');
  ...
  private async promoteToAdr(
    ...
    const filePath = join(adrDir, `${String(nextNum).padStart(4, '0')}-${date}-${slug}.md`);
  ...
  private async promoteToArtifact(
    ...
    const dir = this.config.artifactsDir ?? join(this.config.projectRoot, '.umwelten', 'artifacts');
  ...
  private async promoteToSavedReflection(
    ...
    const dir = this.config.reflectionsDir ?? join(this.config.projectRoot, '.umwelten', 'reflections');
  ...
  private async promoteToUserModel(content: string): Promise<PromotionResult> {
    const filePath = this.config.userModelFile ?? join(this.config.projectRoot, '.umwelten', 'user-model.md');
