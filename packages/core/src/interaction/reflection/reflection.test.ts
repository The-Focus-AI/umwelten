...
import { createDefaultExploration, createVirtualExploration } from '../types/domain-types.js';
...
describe('buildReflectiveInteraction', () => {
  it('creates an Interaction with the question', () => {
    ...
    const interaction = buildReflectiveInteraction([exploration], 'What did we learn?', {
      model,
    });
  ...
  it('pre-populates system context messages', () => {
    ...
    const interaction = buildReflectiveInteraction([exploration], 'Any failures?', {
      model,
    });

    const systemMsgs = interaction.messages.filter((m) => m.role === 'system');
    expect(systemMsgs.length).toBeGreaterThanOrEqual(2);
    expect(systemMsgs.some((m) => (m.content as string).includes('Exploration'))).toBe(true);
  });
  ...
  it('uses the provided model', () => {
    ...
    const interaction = buildReflectiveInteraction([exploration], 'Question?', { model });
  ...
  it('accepts custom instructions', () => {
    ...
    expect(interaction.stimulus.instructions).toContain('Focus on security issues.');
  });
  ...
  it('works with virtual explorations', () => {
    ...
    const interaction = buildReflectiveInteraction([exploration], 'What changed?', { model });
  ...
  it('handles empty explorations array gracefully', () => {
    ...
    const interaction = buildReflectiveInteraction([], 'What happened?', { model });
