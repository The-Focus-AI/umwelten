...
function parseModel(spec: string): ModelDetails {
  ...
  if (idx < 0) throw new Error(`Invalid --model: ${spec} (expected "provider:name")`);
...
export async function runBrowseAction(options: BrowseOptions): Promise<void> {
  try {
    ...
    const { runIntrospectBrowseTui } = await import('@umwelten/ui/tui/introspect/browse.js');
    await runIntrospectBrowseTui({
      projectPath,
      targetPath: primaryTarget,
      sessionsDir: options.sessionsDir ? resolve(options.sessionsDir) : undefined,
      model,
    });
  } catch (err) {
...
registerBrowseOptions(introspectCommand.command('browse')).action(runBrowseAction);

/** `umwelten browse` — top-level, primary entry. */
export const browseCommand = registerBrowseOptions(new Command('browse')).action(
  runBrowseAction
);
