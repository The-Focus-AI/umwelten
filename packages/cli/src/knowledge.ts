...
import { adapterRegistry, initializeAdapters } from '@umwelten/core/interaction/adapters/index.js';
import type { Exploration } from '@umwelten/core/interaction/types/domain-types.js';
...
export async function runKnowledgeCommand(options: KnowledgeCommandOptions): Promise<void> {
  const projectPath = resolve(options.project);
  const sessionsDir = options.sessionsDir ? resolve(options.sessionsDir) : undefined;
  ...
  if (sessionsDir) {
    const { HabitatSessionAdapter } = await import('@umwelten/habitat/adapters/habitat-session-adapter.js');
  ...
  if (projection.explorations.length === 0) {
    ...
    console.log(chalk.dim('Use the project with pi, Claude Code, or Cursor to create sessions first.'));
    return;
  }
  ...
  // ---- 2. List Explorations ----
  listExplorations(projection.explorations, projection.sources);
  ...
  console.log(chalk.dim(`ID: ${selected.id} · ${selected.memberCount} session(s)`));
  console.log();
  ...
  console.log(chalk.dim('(This would call the LLM with the Exploration context.)\n'));
  ...
  for (const msg of contextMessages) {
    const preview = (typeof msg.content === 'string' ? msg.content : JSON.stringify(msg.content)).slice(0, 200);
...
function listExplorations(
  ...
  sources: Array<{ source: string; displayName: string; sessionCount: number; explorationCount: number }>,
): void {
  // Source summary
  for (const source of sources) {
    const color = source.source === 'pi' ? 'magenta' : source.source === 'habitat' ? 'yellow' : 'cyan';
    console.log(chalk.dim(`  ${chalk[color](source.displayName)}: ${source.sessionCount} session(s), ${source.explorationCount} exploration(s)`));
  }
  console.log();

  // Exploration table
  const header = `${chalk.bold('#')}  ${chalk.bold('Name'.padEnd(40))} ${chalk.bold('Source'.padEnd(12))} ${chalk.bold('Msgs')} ${chalk.bold('Date')}`;
  console.log(header);
  ...
  explorations.forEach((exp, i) => {
    ...
    const src = exp.members[0]?.source ?? '?'.padEnd(12);
    const msgs = String(exp.memberCount).padEnd(4);
    const date = exp.created.slice(0, 10);
    const color = exp.kind === 'virtual' ? 'yellow' : exp.kind === 'saved' ? 'green' : 'white';
    console.log(` ${idx}${chalk[color](name)} ${chalk.dim(src.padEnd(12))} ${chalk.dim(msgs)} ${chalk.dim(date)}`);
  });
  console.log();
}

async function selectExploration(explorations: Exploration[]): Promise<Exploration | null> {
  const rl = createInterface({ input: process.stdin, output: process.stdout });

  return new Promise((resolve) => {
    rl.question(chalk.cyan('Select an exploration (number) or press Enter to skip: '), (answer) => {
      rl.close();
...
async function handlePromotion(result: string, projectPath: string): Promise<void> {
  const classification = classifyReflectionAnswer(result);
  const { primary, alternatives } = classification;

  console.log(chalk.bold('\n📊 Classification:'));
  console.log(`  ${chalk.green('Primary:')} ${primary.label} (${Math.round(primary.confidence * 100)}% confidence)`);

  if (alternatives.length > 0) {
    console.log(chalk.dim('  Alternatives:'));
    for (const alt of alternatives.slice(0, 3)) {
      console.log(chalk.dim(`    ${alt.label} (${Math.round(alt.confidence * 100)}%)`));
    }
  }
  console.log();
  ...
  const answer = await new Promise<string>((resolve) => {
    rl.question(
      ...
      chalk.dim('  [1] ') + chalk.white('Save as reflection') + '\n' +
      chalk.dim('  [2] ') + chalk.white(`Promote to ${primary.label}`) + '\n' +
      chalk.dim('  [3] ') + chalk.white('Skip') + '\n' +
...
export const knowledgeCommand = new Command('knowledge')
  .description('Browse project knowledge: discover sessions, reflect, and promote')
