...
import { getAllModels, searchModels } from '@umwelten/core/cognition/models.js';
import { estimateCost } from '@umwelten/core/costs/costs.js';
import { getModelUrl } from '@umwelten/core/providers/index.js';
import { generateLlamaSwapConfig, DEFAULT_CACHE_PATHS } from '@umwelten/core/providers/llamaswap-config.js';
...
type Provider = 'openrouter' | 'ollama' | 'google' | 'github-models' | 'fireworks' | 'minimax' | 'deepinfra' | 'togetherai' | 'lmstudio' | 'llamabarn' | 'all';
...
export const modelsCommand = new Command('models')
  ...
  .option('-p, --provider <provider>', 'Filter by provider (openrouter, ollama, google, github-models, fireworks, minimax, deepinfra, togetherai, lmstudio, llamabarn, all)', 'all')
  .option('--sort <field>', 'Sort by field (name, addedDate, contextLength, cost). If not specified, uses provider default ordering')
