...
const evalCombineCommand = new Command('combine')
  ...
  .action(async (options) => {
    try {
      ...
      // Load suite
      const { loadSuite, buildSuiteReport, buildNarrativeReport } = await import('@umwelten/evaluation/evaluation/combine/index.js');
      const { Reporter } = await import('@umwelten/evaluation/reporting/reporter.js');
