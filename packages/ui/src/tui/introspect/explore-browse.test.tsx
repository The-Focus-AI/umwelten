...
describe('Exploration data helpers', () => {
  it('shows pi metadata in entries', () => {
    ...
    expect(entry.sourceSession.sourceData?.displayName).toBe('Refactor auth module');
    ...
    expect(Object.keys(entry.sourceSession.sourceData!.labels as Record<string, string>).length).toBe(2);
  });
  ...
  it('shows source session membership in entry', () => {
    ...
    expect(entry.exploration.members[0].sourceSessionId).toBe('src-cc-test-001');
      ...
      analyzedIn: [
        {
          ...
          attributedProposals: [
            { kind: 'workflowRule' as const, head: 'Use pnpm', verdict: 'accepted' as const },
          ],
        },
      ],
    });
  ...
  it('supports multiple source types', () => {
    ...
    for (const { source, id } of sources) {
      const session = makeSourceSession({ source: source as SourceSession['source'], id });
      const entry = makeExploreEntry(session);
...
describe('resolveSessionFilePath', () => {
  it('resolves pi session file path from sourceData', async () => {
    const { resolveSessionFilePath } = await import('@umwelten/sessions/introspection/browse.js');
