...
function DigestLiveApp({
	entry,
	model,
	runDigest,
}: DigestLiveAppProps): React.ReactElement {
	const { exit } = useApp();
	...
	useEffect(() => {
		...
		const onProgress = (p: DigestProgress) => {
			...
			setEvents((prev) => [
				...prev,
				{ phase: p.phase, detail: p.detail, at: Date.now() - startedAt },
			]);
		};
	...
	const completedPhases = new Set(events.slice(0, -1).map((e) => e.phase));
		...
		<Box flexDirection="column" padding={1}>
			...
			<Box flexDirection="column" marginTop={1}>
				{PHASE_ORDER.map((phase) => {
					...
					const latestEventForPhase = [...events]
						.reverse()
			...
			{status === "done" && counts && (
				<Box flexDirection="column" marginTop={1}>
					<Text color="green">✓ Digest saved</Text>
					<Text dimColor>
			...
			{status === "cancelled" && (
				<Text color="yellow">
					Cancelled — LLM call may still complete in background.
				</Text>
			)}
			...
			<Box marginTop={1}>
				...
				<Text dimColor>
					{" "}
					to return to browser{" "}
					{status === "running" ? "(digest keeps running)" : ""}
				</Text>
			</Box>
		</Box>
	);
...
export async function runDigestLiveTui(
	opts: RunDigestLiveTuiOptions,
): Promise<void> {
	...
	const app = (
		<DigestLiveApp entry={entry} model={model} runDigest={runDigest} />
	);
