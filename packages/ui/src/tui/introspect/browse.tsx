...
import { ExploreBrowseApp, type ExploreBrowseIntent } from './ExploreBrowseApp.js';
import { buildBrowse, buildExploreBrowse } from '@umwelten/evaluation/introspection/browse.js';
 ...
 */
export async function runExploreBrowseTui(opts: RunBrowseTuiOptions): Promise<void> {
  const { projectPath: rawProject, targetPath: rawTarget, sessionsDir, model } = opts;
  const projectPath = resolve(rawProject);
  const targetPath = resolve(rawTarget);

  while (true) {
    const { entries, runs } = await buildExploreBrowse({ projectPath, sessionsDir });
    ...
    if (intent.kind === 'transcript') {
      if (!entry.filePath) {
        console.log(chalk.yellow('Transcript not available — file path unknown for this session.'));
        continue;
      }
      const { runSessionTui } = await import('../index.js');
      await runSessionTui({ projectPath, filePath: entry.filePath, hasStdin: false });
      continue;
    }
    ...
    if (intent.kind === 'detail') {
      ...
      await runDigestDetailTui({ projectPath, targetPath, entry: syntheticEntry, model });
      continue;
    }
    ...
    if (intent.kind === 'beats') {
      if (!entry.filePath) {
        console.log(chalk.yellow('Beats not available — file path unknown for this session.'));
        continue;
      }
      const { runBeatsTui } = await import('./beats.js');
      try {
        ...
        console.error(chalk.red(`[beats] failed: ${err instanceof Error ? err.message : String(err)}`));
      }
      continue;
    }
  }
