...
async function createHabitatSession(
  ...
  await writeFile(join(sessionDir, 'meta.json'), JSON.stringify(meta, null, 2), 'utf-8');
  ...
  await writeFile(join(sessionDir, 'transcript.jsonl'), [header, ...lines].join('\n') + '\n', 'utf-8');
...
describe('HabitatSessionAdapter', () => {
  ...
  it('registers with the adapter registry correctly', async () => {
    const { adapterRegistry } = await import('@umwelten/core/interaction/adapters/adapter.js');
    const adapter = new HabitatSessionAdapter(testDir);
