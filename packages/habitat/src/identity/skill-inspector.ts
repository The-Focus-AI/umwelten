...
const ENV_NOISE = new Set([
  "PATH", "HOME", "USER", "SHELL", "PWD", "LANG", "TERM", "TMPDIR", "EDITOR",
  "OLDPWD", "DISPLAY", "HOSTNAME", "LC_ALL", "LD_LIBRARY_PATH", "LOGNAME",
  "MAIL", "PS1", "PS2", "TZ", "XDG_CONFIG_HOME", "XDG_DATA_HOME", "XDG_CACHE_HOME",
  "NODE_ENV", "DEBUG", "CI", "GITHUB_ACTIONS",
]);

const CLI_NOISE = new Set([
  "echo", "cat", "ls", "cd", "pwd", "exit", "true", "false", "test", "[",
  "if", "then", "else", "elif", "fi", "for", "while", "do", "done", "set",
  "unset", "export", "source", ".", "read", "shift", "return",
  "printf", "sleep", "trap", "wait", "eval", "exec", "function",
  "case", "esac", "select", "until", "in", "let", "local", "declare", "readonly",
  "typeset", "alias", "command", "type", "which", "hash", "history", "fg", "bg",
  "jobs", "kill", "umask", "ulimit", "cd", "pushd", "popd", "dirs", "complete",
  "env", "tee", "head", "tail", "sort", "uniq", "wc", "tr", "cut", "paste",
  "awk", "sed", "grep", "find", "xargs", "rm", "cp", "mv", "mkdir", "rmdir",
  "touch", "ln", "chmod", "chown", "chgrp", "stat", "df", "du", "free", "ps",
  "top", "uname", "hostname", "id", "whoami", "groups", "passwd", "su", "sudo",
]);

const CODE_EXTS = new Set([".ts", ".tsx", ".js", ".mjs", ".cjs", ".jsx", ".py"]);
...
function scanCodeFile(content: string, rel: string, env: Map<string, Hit>, cli: Map<string, Hit>): void {
  ...
  for (const m of content.matchAll(/os\.environ(?:\.get)?\((?:"|')([A-Z][A-Z0-9_]+)(?:"|')/g)) {
    addEnv(env, m[1], rel);
  }
  for (const m of content.matchAll(/os\.environ\[(?:"|')([A-Z][A-Z0-9_]+)(?:"|')\]/g)) {
    addEnv(env, m[1], rel);
  }
  for (const m of content.matchAll(/os\.getenv\((?:"|')([A-Z][A-Z0-9_]+)(?:"|')/g)) {
    addEnv(env, m[1], rel);
  }
...
function scanShellFile(content: string, rel: string, env: Map<string, Hit>, cli: Map<string, Hit>): void {
  // ${NAME} and $NAME
...
function scanMarkdown(content: string, rel: string, env: Map<string, Hit>): void {
  // Pull bullet items under "Required environment variables", "Env vars", or "Environment".
  const lines = content.split("\n");
  let inEnvSection = false;
  for (const line of lines) {
    if (/^#{1,6}\s+(?:required\s+)?(?:environment(?:\s+variables?)?|env\s*vars?)\b/i.test(line)) {
      inEnvSection = true;
      continue;
    }
    if (/^#{1,6}\s+/.test(line)) {
 ...
 */
export async function inspectSkill(skillDir: string): Promise<AgentRequirements> {
  ...
  for (const sub of SCAN_DIRS) {
    ...
    let exists = false;
    try { exists = (await stat(root)).isDirectory(); } catch {}
    if (!exists) continue;
 ...
 * for each env var / CLI tool. Used by computeRequirements() to aggregate
 * across skills + agents.
 */
export function mergeRequirements(parts: AgentRequirements[]): AgentRequirements {
  ...
  for (const part of parts) {
    for (const e of part.envVars) {
      ...
      if (!existing) { env.set(e.name, e); continue; }
    ...
    for (const c of part.cliTools) {
      ...
      if (!existing) { cli.set(c.name, c); continue; }
