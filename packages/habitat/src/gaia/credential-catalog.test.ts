...
describe("CredentialCatalog", () => {
  ...
  it("lists credentials by capability", async () => {
    await catalog.add(makeEntry({ name: "gh-read", capabilities: ["github:read"] }));
    await catalog.add(makeEntry({ name: "gh-write", capabilities: ["github:write"] }));
    await catalog.add(makeEntry({ name: "qb-read", capabilities: ["quickbooks:read"] }));
  ...
  it("lists credentials by provider", async () => {
    ...
    await catalog.add(makeEntry({ name: "qb-key", provider: "intuit/quickbooks" }));
