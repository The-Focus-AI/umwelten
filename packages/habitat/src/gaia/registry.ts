...
export class GaiaRegistryManager {
  ...
  private ensureLoaded(): void {
    if (!this.loaded) throw new Error("Registry not loaded — call load() first");
  }
  ...
  async update(
    id: string,
    updates: Partial<Pick<GaiaHabitatEntry, "name" | "config" | "secretBindings" | "containerPort">>,
    ...
    if (updates.secretBindings !== undefined) entry.secretBindings = updates.secretBindings;
    if (updates.containerPort !== undefined) entry.containerPort = updates.containerPort;
