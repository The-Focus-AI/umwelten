		...
		SearchToolsContext
{
	...
	private async buildStimulus(): Promise<Stimulus> {
		...
		if (!this.options.skipSkills) {
			...
			if (
				!defaultSkillsDirs.includes("./.agents/skills") &&
				!defaultSkillsDirs.includes(".agents/skills")
			) {
				defaultSkillsDirs.push("./.agents/skills");
			}
			const skillsDirsResolved = defaultSkillsDirs.map((d) =>
				join(this.workDir, d),
			);
	...
	addTool(name: string, tool: Tool): void {
		this.toolRegistry.addTool(name, tool);
	}
	...
	addToolSet(toolSet: ToolSet): void {
		this.toolRegistry.addToolSet(toolSet);
	}

	getSkills(): SkillDefinition[] {
		return this.toolRegistry.getSkills();
	}
	 ...
	 *
	 * When `catalog` is provided, env vars discovered in skill source are
	 * cross-referenced against the credential catalog to produce capability
	 * hints in the result.
	 */
	async computeRequirements(catalog?: InspectorCatalog): Promise<{
		...
		const { loadSkillsFromDirectory } = await import(
			"@umwelten/core/stimulus/skills/index.js"
		);
		...
		const skills = await Promise.all(
			all.map(async (s) => ({
				name: s.name,
				path: s.path,
				requirements: await inspectSkill(
					s.path,
		...
		const agents = (this.config.agents ?? [])
			...
			.map((a) => ({
				id: a.id,
				requirements: a.requirements as AgentRequirements,
			}));
	...
	/**
	 * Record a capability gap reported at runtime (e.g., a tool or sub-agent
	 * hit an auth error because it lacked a credential).
	 */
	recordCapabilityGap(gap: CapabilityGap): void {
		this.capabilityGaps.push(gap);
	}

	/**
	 * Return all recorded capability gaps. Gaps are in-memory and reset
