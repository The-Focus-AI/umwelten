...
import { DockerTestResult } from './docker-generator.js';
...
export class ReportGenerator {
  ...
  static generateDockerResultsReport(
    responsesDir: string, 
    dockerDir: string, 
    dockerResults: DockerTestResult[]
  ): void {
...
`;
    ...
    for (const file of responseFiles) {
      ...
      try {
        ...
        const buildStatusEmoji = dockerResult?.buildSuccess ? '✅' : '❌';
        const testStatusEmoji = dockerResult?.testSuccess ? '✅' : '❌';
        const codeQualityEmoji = codeQuality === 'good' ? '✅' : codeQuality === 'syntax_errors' ? '⚠️' : '❌';
        
        report += `| ${modelName} | ${buildStatusEmoji} ${dockerResult?.buildSuccess ? 'Success' : 'Failed'} | ${testStatusEmoji} ${dockerResult?.testSuccess ? 'Success' : 'Failed'} | ${codeQualityEmoji} ${codeQuality === 'good' ? 'Good' : codeQuality === 'syntax_errors' ? 'Syntax Errors (Fixed)' : 'Poor'} | ${notes} |\n`;
    ...
    report += `
## Statistics
- **Successful Builds**: ${successfulBuilds}/${responseFiles.length} (${Math.round(successfulBuilds/responseFiles.length*100)}%)
- **Successful Tests**: ${successfulTests}/${responseFiles.length} (${Math.round(successfulTests/responseFiles.length*100)}%)
- **Good Code Quality**: ${goodCodeQuality}/${responseFiles.length} (${Math.round(goodCodeQuality/responseFiles.length*100)}%)

## Key Findings

### ✅ Working Models
${dockerResults.filter(r => r.testSuccess).map(r => `- **${r.modelName}**: Generated valid TypeScript code that compiles and runs`).join('\n')}

### ❌ Failed Models  
${dockerResults.filter(r => !r.testSuccess).map(r => `- **${r.modelName}**: ${r.error || 'Failed to generate working code'}`).join('\n')}
