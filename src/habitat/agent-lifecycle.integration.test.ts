...
describe("Agent Lifecycle (integration)", () => {
  ...
  it("should clone a repo and register an agent (agent_clone)", async () => {
    ...
    if (result.error) {
      throw new Error(
        `agent_clone failed: ${result.error} — ${result.message}`,
      );
    }
    ...
    // Note: With Bridge system, repo is cloned INSIDE container, not on host filesystem
  ...
  it("should execute a command in a Dagger container", async () => {
    ...
    if (result1.stderr?.includes("GraphQLRequestError")) {
      console.warn(
        "Dagger SDK version issue detected — skipping container assertions",
      );
  ...
  it("should serve the Gaia HTTP API", async () => {
    ...
    try {
      ...
      if (sessData.sessions.length > 0) {
        ...
        if (agentSession) {
          // GET /api/sessions/:id
          const sessShowRes = await fetch(
            `${base}/api/sessions/habitat-agent-trmnl-image-agent`,
          );
          expect(sessShowRes.status).toBe(200);
          const sessShowData = (await sessShowRes.json()) as any;
          expect(sessShowData.sessionId).toBe(
            "habitat-agent-trmnl-image-agent",
          );
        }
      }
