...
import type { SessionBrowserEntry } from '../../../introspection/browse.js';
import { loadDigest } from '../../../introspection/browse.js';
import type { SessionDigest } from '../../../interaction/analysis/analysis-types.js';
  ...
  return (
    <Box flexDirection="column" height={rows}>
      ...
      <Box borderStyle="single" borderColor="gray" paddingX={1}>
        ...
        <Text dimColor>{`  · ${entry.source} · ${entry.messageCount} msgs · ${entry.gitBranch ?? '-'} · modified ${formatAge(entry.modifiedMs)}`}</Text>
      ...
      <Box borderStyle="single" borderColor="gray" paddingX={1}>
        {TABS.map((t, i) => {
          ...
          return (
            <Text key={t} color={active ? 'cyan' : undefined} bold={active} dimColor={!active}>
              {` ${i + 1} ${TAB_LABELS[t]} `}
            </Text>
          );
        })}
        {!digest && (
          <Text color="yellow">{'  (no digest yet — press D)'}</Text>
        )}
      </Box>
...
function buildOverviewLines(digest: SessionDigest, width: number): string[] {
  ...
  if (digest.metrics) {
    ...
    if (digest.metrics.estimatedCost) {
      lines.push(`  Est. cost:     $${digest.metrics.estimatedCost.toFixed(3)}`);
    }
    lines.push('');
  }
...
function buildBeatsLines(digest: SessionDigest, width: number): string[] {
  if (!digest.beats?.length) {
    return ['(no beats in this digest)', '', 'This digest was generated without beat extraction.'];
  }
  const lines: string[] = [];
  for (const b of digest.beats) {
    ...
    lines.push('');
    lines.push('USER');
    lines.push(...wrap(b.userRequest || '(no prompt)', width - 2).map((l) => `  ${l}`));
    ...
    if (b.outcome) {
      lines.push('');
      lines.push('OUTCOME');
      lines.push(...wrap(b.outcome.slice(0, 800), width - 2).map((l) => `  ${l}`));
    }
    if (b.narrative) {
      lines.push('');
    ...
    if (b.keyFacts?.length) {
      ...
      for (const f of b.keyFacts) {
        lines.push(...wrap(`  · ${f}`, width).map((l, i) => (i === 0 ? l : `    ${l}`)));
      }
    }
  }
  return lines;
...
function buildPhasesLines(digest: SessionDigest, width: number): string[] {
  if (!digest.phases?.length) {
    return ['(no phases)', '', 'Phases are detected from beats; a digest without beats cannot have phases.'];
  }
  const lines: string[] = [];
  for (const [i, p] of digest.phases.entries()) {
    ...
    lines.push('');
    lines.push(...wrap(p.description || '(no description)', width - 2).map((l) => `  ${l}`));
  }
  return lines;
}
  ...
  if (digest.extractedFacts?.length) {
    ...
    for (const [type, facts] of byType.entries()) {
      ...
      for (const text of facts) {
        lines.push(...wrap(`  · ${text}`, width).map((l, i) => (i === 0 ? l : `    ${l}`)));
      }
      lines.push('');
    }
  }
...
export async function runDigestDetailTui(opts: RunDigestDetailTuiOptions): Promise<void> {
  const { entry, model } = opts;
  ...
  const initialDigest = await loadDigest(entry.id);
  ...
  const app = (
    <DetailApp entry={entry} initialDigest={initialDigest} onRunDigest={onRunDigest} />
  );
