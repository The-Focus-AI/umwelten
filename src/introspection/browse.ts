...
import { loadRun, listRuns, readDecisions, tallyRun, normalizeKey } from './storage.js';
import type {
  IntrospectionRun,
  DecisionLogEntry,
  DecisionKind,
...
export function getDigestPath(sessionId: string): string {
  return join(homedir(), '.umwelten', 'digests', 'sessions', `${sessionId}.json`);
}
...
export interface SessionBrowserEntry {
  ...
  analyzedIn: Array<{
    runId: string;
    runCreatedAt: string;
    tally: { total: number; accepted: number; skipped: number; pending: number };
...
function attributeProposalToSession(
  ...
  for (const f of run.result.architectureFacts)
    if (check(f.evidence)) matches.push({ kind: 'architectureFact', head: f.fact });
...
export async function buildBrowse(
  ...
  const ccEntries: Array<Omit<SessionBrowserEntry, 'analyzedIn' | 'modifiedSinceAnalysis' | 'everAnalyzed'>> = [];
  ...
  if (sessionsDir) {
    ...
    for (const name of names) {
      ...
      const { parseSessionFileMetadata } = await import(
        '../interaction/persistence/session-parser.js'
      );
...
export type StatusFilter = 'all' | 'unanalyzed' | 'pending' | 'decided' | 'fresh' | 'digested' | 'undigested';
