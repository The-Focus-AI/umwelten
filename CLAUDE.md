...
- `provision/` — **the container's provisioning decision** (#269). `planProvision(config, volumeState)` is pure and returns a `ProvisionPlan` — clone vs. fast-forward the Owned repo, which mounts are new, toolchain/dependency installs, skills restore vs. install — without performing any of it. `execute.ts` runs the plan behind injected effects; `provision.ts` is the CLI `entrypoint.sh` invokes once (`--dry-run` prints the plan and exits). The shell keeps only GitHub-token resolution and the `insteadOf` git config, which this process inherits.
