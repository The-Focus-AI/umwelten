...
- `reflection/` — `buildReflectiveInteraction(explorations, question, opts)` constructs an Interaction whose system prompt is a "reflective analyst" plus exploration context. Not a new runner — just a configured Interaction. Also `runFanout(opts)` (`fanout.ts`): runs a batch of **Probes** concurrently over one Interaction's current state after a turn — *annotations* (verbatim history + one short question, so every probe shares a prefix) and *baselines* (a compaction strategy, returning a `replacement` context you can continue from). Records latency/tokens/cost/`sharedPrefix` per probe; a failing probe returns an error result rather than throwing. Driven by `examples/context-explorer/`; see `docs/guide/turn-fanout.md` and ADR 0010.
...
Consumed by `@umwelten/sessions/introspect` (projection). **Reflection + promotion have no CLI surface** — `classifyReflectionAnswer` / `PromotionRouter` are programmatic only; the nearest command is `sessions digest knowledge`, which reports knowledge accumulated by digestion rather than driving promotion. See `docs/guide/reflections-and-knowledge.md`. The pipeline is a candidate for splitting out into `@umwelten/knowledge` later — see Wave H in `docs/architecture/system-map-2026-05.md`.
...
- `provision/` — **the container's provisioning decision** (#269, #276). `planProvision({config, volume, intent})` is pure and returns a `ProvisionPlan` — clone vs. fast-forward the Owned repo, which mounts are new, toolchain/dependency installs, skills restore vs. install — without performing any of it. Two intents: **`start`** (the boot path; on a warm volume it plans *no* git, installs or skills work, so wake latency does not scale with mount count) and **`refresh`** (pull the Owned repo and every Mounted repo, reinstall, re-restore). `execute.ts` runs the plan behind injected effects; `provision.ts` is the CLI `entrypoint.sh` invokes once (`--refresh` for the refresh intent, `--dry-run` prints the plan and exits). The shell keeps only GitHub-token resolution and the `insteadOf` git config, which this process inherits. A Dormant habitat is refreshed by Gaia leaving a `.needs-refresh` mark on its volume (`provision/stale.ts`); the next start promotes itself to a refresh and clears the mark. Gaia's side is `gaia-tools/refresh.ts` (`refresh_habitat`).
...
- `commonOptions.ts` — `addCommonOptions` for `-p`/`-m` etc. Currently consumed by `run.ts`/`chat.ts`/`tools.ts`; the bigger subcommands (`habitat`, `mcp`) define their own.
