...
pnpm run cli -- eval report --id my-evaluation --format json | jq '.cost'

# Export cost data for analysis
pnpm run cli -- eval report --id cost-analysis --format csv --output costs.csv
...
pnpm run cli -- eval report --id product-description-cost --format markdown
```
...
pnpm run cli -- eval report --id healthcare-ai-analysis --format csv --output healthcare-costs.csv
```
...
pnpm run cli -- eval report --id document-batch-costs --format json
```
...
pnpm run cli -- eval report --id cost-estimate-test --format json | jq '.cost'
...
for eval_id in $(pnpm run cli -- eval list --json | jq -r '.[].id'); do
  pnpm run cli -- eval report --id "$eval_id" --format json >> all-costs.jsonl
done
...
pnpm run cli -- eval list --json | jq '
...
pnpm run cli -- eval list --json | jq -r '
