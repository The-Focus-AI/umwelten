...
pnpm run cli -- models --view info --id "google:gemini-3-flash-preview"
```
...
pnpm run cli -- run --provider ollama --model gemma3:12b "Explain quantum computing in simple terms"

# With a specific model
pnpm run cli -- run --provider google --model gemini-3-flash-preview "Write a short story about a robot learning to paint"
```
...
```bash
pnpm run cli -- run [options] "your prompt here"
```

**Common Options**:
- `--provider`: Specify the AI provider (`google`, `ollama`, `openrouter`, `lmstudio`, `llamabarn`, `llamaswap`, `github-models`, `fireworks`, `minimax`)
...
pnpm run cli -- run --provider ollama --model gemma3:12b "What is machine learning?"
...
pnpm run cli -- eval report --id fibonacci-comparison --format markdown
```
...
pnpm run cli -- run --provider google --model gemini-3-flash-preview "Quick analysis task"

# Highest quality reasoning
pnpm run cli -- run --provider google --model gemini-2.5-pro-exp-03-25 "Complex reasoning task"
...
```bash
# General purpose
pnpm run cli -- run --provider ollama --model gemma3:12b "General task"

# Code generation
pnpm run cli -- run --provider ollama --model codestral:latest "Write Python code for sorting"
...
pnpm run cli -- run --provider openrouter --model openai/gpt-4o "High-quality analysis"

# Claude 3.7 Sonnet
pnpm run cli -- run --provider openrouter --model anthropic/claude-3.7-sonnet "Detailed reasoning task"
```
...
pnpm run cli -- run --provider ollama --model gemma3:12b "Hello, world!"

# Add temperature control
pnpm run cli -- run --provider ollama --model gemma3:12b --temperature 0.5 "Hello, world!"
...
```bash
# Check model costs
pnpm run cli -- models costs --sort prompt

# Run with cost tracking
pnpm run cli -- run --provider openrouter --model openai/gpt-4o "Your prompt" --verbose
```
...
1. **"Model not found"**: Check available models with `pnpm run cli -- models --provider <provider>`
...
pnpm run cli -- chat --help
pnpm run cli -- eval --help

# Provider-specific help
pnpm run cli -- models --help
```
