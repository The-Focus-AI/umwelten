...
const fireworksModel: ModelDetails = {
  name: '<fireworks-model-id>',
  provider: 'fireworks'
};

// MiniMax (requires MINIMAX_API_KEY)
const minimaxModel: ModelDetails = {
  name: 'MiniMax-M2.5',
  provider: 'minimax'
};
...
// llama-swap local models (requires llama-swap at localhost:8080 by default)
...
interface ModelDetails {
  ...
  provider: string;      // Provider ('google', 'ollama', 'openrouter', 'lmstudio', 'llamabarn', 'llamaswap', 'github-models', 'fireworks', 'minimax')
