...
export function registerGenericTools(server: Server, db: Database) {
	server.tool('list_tables', {
		...
		handler: async () => {
			...
			return { content: [{ type: 'text', text: JSON.stringify(rows.map((r: any) => r.name)) }] };
		}
	});
	...
	server.tool('run_sql', {
		...
		handler: async (args: any) => {
			if (!isSelectOnly(args.sql)) throw new Error('Only SELECT queries are allowed');
