...
export async function detectHeaderAndData(
	...
	// Prefer stable window with maximum column count; tie-breaker: earliest
	...
	// Current-line heuristic (if previous not selected)
	...
	if (ambiguous && useLLM) {
		const llm = createLLMClient(modelId);
		...
		const json = await llm.decideHeaderBoundary(snippet, delimiter);
		if (json && Number.isInteger(json.header_row_index) && Number.isInteger(json.data_start_row_index)) {
