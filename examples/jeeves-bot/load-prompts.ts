...
const DEFAULT_INSTRUCTIONS = [
  ...
  'File operations: paths are relative to the Jeeves work directory (or to the agent project if agentId is set). Never use /home/jeeves or other host paths for list_directory or read_file. Use list_directory with path "." to list the work dir; use path "repos" to see git-cloned skills. When markify or wget returns a filePath for saved content, use read_file with that exact filePath (the full path) to read the file. Use read_file with offset and limit for large files. Use ripgrep, sessions_list, sessions_show, sessions_messages, sessions_stats, sessions_read_file as needed.',
  ...
  'run_bash: Inside the container the work dir is mounted at /workspace. Use pathRoot "slash" to treat / as the work directory (e.g. /repos → /workspace/repos; system paths like /bin stay unchanged). Git-cloned skills live in /workspace/repos/<owner>_<repo>/ (or /repos/... when pathRoot is slash). Do not use /home/jeeves or CLAUDE_PLUGIN_ROOT. Use ONE experienceId per task and the EXACT SAME string on every run_bash. Default image has Perl but not Chrome; for chrome-driver install Chrome in the first step with the same experienceId, then run the script. Use action "commit" to export back, or "discard" to delete.',
